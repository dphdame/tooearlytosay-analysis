...
- A **compound treatment** at the same cutoff changes the local fit to **1.18**. The running variable
  is uniform by construction, yet the design still estimates the combined discontinuity rather than
  the target program's **0.75** effect. The reported ±0.1 count-balance screen is descriptive; it is
  not a McCrary density test and cannot diagnose a second treatment at the cutoff.
...
| `data/results.json` | Machine-readable output from the fixed seed, regenerated by the harness. |
...
python code/rdd_scatter.py           # writes figures/rdd-scatter.png
```

Both commands resolve paths from the package directory, so they work from the repository root or
another current working directory. The harness exits nonzero if its local-linear recovery checks fail.
